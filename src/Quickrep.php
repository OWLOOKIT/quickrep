...
class Quickrep
{
    ...
    public static function reportsIn($directory)
    {
        ...
        foreach ((new Finder)->in($directory)->files() as $report) {
            ...
            try {
                if ( is_subclass_of( $report, QuickrepReport::class ) &&
                    !(new ReflectionClass( $report ))->isAbstract() ) {
                    $reports[] = $report;
                }
            } catch ( \Exception $e ) {
                throw new \Exception($e->getMessage().". If you recently made changes to the contents of your Quickrep reports directory, you may have do run `composer dump-autoload`");
            }
        }
