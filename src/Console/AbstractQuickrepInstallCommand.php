...
abstract class AbstractQuickrepInstallCommand extends Command
{
    /**
     ...
     */
    protected static $asset_target_path = 'vendor/Owlookit/quickrep';
    ...
    protected function createDirectories()
    {
        ...
        if (!is_dir($directory = resource_path('views/quickrep'))) {
            mkdir($directory, 0755, true);
        }

        if (!is_dir($directory = resource_path('views/quickrep/layouts'))) {
            mkdir($directory, 0755, true);
        }
    }

    /**
     * Export the quickrep views.
     *
     * @return void
     */
    ...
    protected function exportConfig()
    {
        ...
        if ($doCopy) {
            ...
            $this->mergeConfigFrom(
                config_path($filename),
                basename($filename, ".php") // use the key as the filename without extension
            );
        } else {
            ...
            if (file_exists(config_path($filename))) {
                ...
                foreach ($newConfig as $key => $value) {
                    if (!isset($currentConfig[$key])) {
                        ...
                        $this->error(
                            "Your current configuration file is missing required setting `{$key}`. Please refer to the package config `$config_file` to copy the default setting value."
                        );
                    }
                }
            }
    ...
    {
        $this->info("Installing Quickrep Assets");
